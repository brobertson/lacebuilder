...
def count_things(treeIn, totals):
    words = treeIn.xpath(
        "//html:span[@class='ocr_word'] | //html:span[@class='ocrx_word']"
        "| //span[@class='ocr_word']",
    ...
    for word in words:
        ...
        if (word.text) and (is_greek_string(word.text)):
            ...
            if word.get("data-spellcheck-mode") == "None":
                totals["no_spellcheck_greek_words"] = (
                    totals["no_spellcheck_greek_words"] + 1
                )
            if word.get("data-manually-confirmed") == "true":
...
def assess(dir_in_path):
    try:
        dir_in = sys.argv[1]
        # print dir_in
        dir_in_list = os.listdir(dir_in_path)
    ...
    try:
        ...
        greek_accuracy = 100 - (
            100 * totals["no_spellcheck_greek_words"] / totals["greek_word_count"]
        )
        if totals["word_count"] == 0:
            accuracy = 0
            greek_accuracy = 0
        verified_greek_word_pct = (
            100 * totals["verified_greek_words"] / totals["greek_word_count"]
        )
        verified_words_pct = 100 * totals["verified"] / totals["word_count"]
        print(
            (
                "accuracy %02d%%, Greek acc. %02d%%; "
                "completed %02d%%, Greek completed %02d%%"
                % (
                    accuracy,
                    greek_accuracy,
                    verified_words_pct,
        ...
        return greek_accuracy
    except ZeroDivisionError:
        print(
            (
                "accuracy %02d%%, Greek acc. %02d%%; "
                "completed %02d%%, Greek completed %02d%%" % (0, 0, 0, 0)
            )
        )
        print(
            (
                "there was an error in calculating these results. "
                "Here is a dump of the totals:",
                totals,
            )
        )
