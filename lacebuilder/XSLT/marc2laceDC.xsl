...
<xsl:stylesheet version="2.0" xmlns:marc="http://www.loc.gov/MARC21/slim"
  ...
  <xsl:variable name="progname">
    <xsl:text>marc2laceDC.xsl</xsl:text>
  </xsl:variable>
  ...
  <xsl:template match="marc:record">
    ...
    <xsl:variable name="sirsiRecordID">
      <xsl:value-of select="marc:controlfield[matches(@tag, '001')]"/>
    </xsl:variable>
    <xsl:for-each select="*:datafield[matches(@tag, '999')]">
      <xsl:variable name="fileName">
        <xsl:value-of select="concat(*:subfield[matches(@code, 'i')], '.xml')"/>
      </xsl:variable>
      <xsl:result-document href="{$sirsiRecordID}/{$fileName}">
        ...
        <lace:imagecollection xmlns:lace="http://heml.mta.ca/2019/lace"
          ...
          <dc:identifier>
            <xsl:value-of select="*:subfield[matches(@code, 'i')]"/>
          </dc:identifier>
          <dc:contributor>
            <xsl:text>University of Virginia Library</xsl:text>
          </dc:contributor>
          <dc:source>
            <xsl:value-of select="$sirsiRecordID"/>
          </dc:source>
          <dc:source>
            <xsl:value-of select="*:subfield[matches(@code, 'a')]"/>
          </dc:source>
          ...
          <!-- Exclude 59x -->
          <xsl:for-each
            select="../marc:datafield[500 &lt;= @tag and @tag &lt;= 589][not(@tag = 506 or @tag = 530 or @tag = 540 or @tag = 546)]">
            <dc:description>
              <xsl:value-of select="marc:subfield[@code = 'a']"/>
            </dc:description>
          </xsl:for-each>
