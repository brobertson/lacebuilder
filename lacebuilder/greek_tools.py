...
def is_hebrew_char(char):
    ...
    return bool(re.match(r"([\u05D0-\u05F2])", char, re.UNICODE))
...
def is_greek_capital(char):
    ...
    return bool(re.match(r"([\u0391-\u03A9])", char, re.UNICODE))
...
def is_number(string_in):
    # for the purposes of OCR, we include Latin capital O and 
    #Latin capital I as digits :-)
    # the point here isn't to clean, but rather to discern if, in a Latin OCR
    #output context, the word is likely a number
    import re

    allowed_punct = (
        "\.(),;"  # what punctuation do you want to include as part of a number?
    )
    allowed_chars = "OIi"
...
def preprocess_word(word):
    ...
    circumflex = str("\N{COMBINING GREEK PERISPOMENI}")
    ...
    word = re.sub(
        r"^" + apostrophe + r"([I1ΙιΕEAP])(.*)$", r"\1" + smooth_breathing + r"\2", word
    )
    ...
    # remove internal punctuation. Uses negative lookahead and 
...
def strip_accents(stringIn):
    ...
    return "".join(
        c
        for c in unicodedata.normalize("NFD", stringIn)
